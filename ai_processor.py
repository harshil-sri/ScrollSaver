...
def process_media(file_paths: list[str], category: str, content_type: str, custom_instructions: str = None, caption_text: str = "", extract_frames: bool = False) -> dict:
    ...
    if audio_paths:
        ...
        for ap in audio_paths:
            ...
            with open(ap, "rb") as audio_file:
                ...
                whisper_prompt = "The audio may be in English, Hindi, or a mix. If English, transcribe normally in English. If Hindi, transcribe in Hinglish (Roman/English alphabet, jaise main abhi likh raha hoon)."
                if custom_instructions and custom_instructions.lower() != "skip":
                    whisper_prompt += f" Pay special attention to these terms: {custom_instructions}"
    ...
    system_prompt += "\nWARNING: The audio transcript was generated by an AI and may contain phonetic misspellings of new tech tools (e.g., 'CDANCE' instead of 'Seedance', 'Cloud' instead of 'Claude', etc). If a tool name looks like a phonetic misspelling, intelligently deduce the correct spelling."
